...
class Current_Prayers_Venture_Endpoints {
    ...
    private static function get_personal_benchmarks() {
        ...
        $contacts_previous = $wpdb->get_var(
            $wpdb->prepare("
            SELECT COUNT(DISTINCT(object_id))
            FROM $wpdb->dt_activity_log a
            INNER JOIN $wpdb->postmeta as type ON ( object_id = type.post_id AND type.meta_key = 'type' AND type.meta_value != 'user' )
        ...
        $milestones_current = $wpdb->get_var(
            $wpdb->prepare("
            SELECT COUNT(DISTINCT(object_id))
            FROM $wpdb->dt_activity_log a
            INNER JOIN $wpdb->postmeta as type ON ( object_id = type.post_id AND type.meta_key = 'type' AND type.meta_value != 'user' )
        ...
        $milestones_previous = $wpdb->get_var(
            $wpdb->prepare("
            SELECT COUNT(*)
            FROM $wpdb->dt_activity_log a
            INNER JOIN $wpdb->postmeta as type ON ( object_id = type.post_id AND type.meta_key = 'type' AND type.meta_value != 'user' )
    ...
    public static function milestones() {
        ...
        $res = $wpdb->get_results($wpdb->prepare(
            "SELECT b.meta_value as milestones, count( a.ID ) as value
             FROM $wpdb->posts as a
               JOIN $wpdb->postmeta as b
                 ON a.ID=b.post_id
